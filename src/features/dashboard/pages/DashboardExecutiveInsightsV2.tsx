  ...
  return (
    <section dir={ar ? 'rtl' : 'ltr'} className="space-y-6" data-testid="dashboard-executive-insights">
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
        ...
        <Card><div className="mb-5 flex items-center gap-2"><AlertTriangle className="h-5 w-5 text-amber-500" /><h3 className="font-black text-ui-text">{ar ? 'تنبيهات تشغيلية' : 'Operational alerts'}</h3></div><Link to="/inventory" className="flex items-center gap-3 rounded-2xl bg-amber-500/10 p-3"><Boxes className="h-5 w-5 text-amber-500" /><div className="flex-1"><p className="text-sm font-bold text-ui-text">{ar ? 'مخزون منخفض' : 'Low stock'}</p><p className="text-xs text-ui-muted">{formatNumber(lowStockCount)} {ar ? 'منتج يحتاج مراجعة' : 'items need review'}</p></div><ArrowUpRight className="h-4 w-4 text-ui-subtle" /></Link></Card>
      </div>
