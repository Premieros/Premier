...
function selectReport(report: string) {
  document.querySelector<HTMLButtonElement>(`button[data-report-type="${report}"]`)?.click();
}
...
export function ReportDeepLinkPage() {
  ...
  const refreshColumns = () => {
    ...
    const saved = localStorage.getItem(`premier.report.columns.${report}`);
    const savedValues = saved ? JSON.parse(saved) as boolean[] : [];
    setColumns(headers);
  ...
  useEffect(() => {
    ...
    cells.forEach((row) => row.querySelectorAll<HTMLElement>(':scope > *').forEach((cell, i) => {
      cell.style.display = visible[i] === false ? 'none' : '';
    }));
    const report = document.querySelector<HTMLSelectElement>('[data-testid="report-type-select"]')?.value || 'report';
    ...
    <div className="relative">
      ...
      <div className="mb-4 rounded-ui-xl border border-ui-border bg-ui-surface shadow-ui overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-ui-border px-4 py-3">
          <div>
            <div className="flex items-center gap-2 text-sm font-bold text-ui-text"><FileText className="h-4 w-4 text-ui-primary" />{lang === 'ar' ? 'مركز التقارير' : 'Reports Center'}</div>
            <p className="mt-0.5 text-xs text-ui-muted">{lang === 'ar' ? 'كل التقارير في مكان واحد بدون ازدحام' : 'All reports, organized without clutter'}</p>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={() => setCustomizeOpen((v) => !v)} className="flex items-center gap-2 rounded-ui-lg border border-ui-border bg-ui-page-alt px-3 py-2 text-xs font-semibold text-ui-text hover:bg-ui-primary-soft hover:text-ui-primary"><Settings2 className="h-4 w-4" />{lang === 'ar' ? 'تخصيص الأعمدة' : 'Customize columns'}</button>
        ...
        <div className="flex gap-1 overflow-x-auto border-b border-ui-border px-3 py-2">
          {GROUPS.map((item) => { const Icon = item.icon; const active = item.key === activeGroup; return <button key={item.key} onClick={() => setActiveGroup(item.key)} className={`flex shrink-0 items-center gap-2 rounded-ui-lg px-3 py-2 text-xs font-semibold transition ${active ? 'bg-ui-primary text-ui-primary-fg' : 'text-ui-muted hover:bg-ui-page-alt hover:text-ui-text'}`}><Icon className="h-4 w-4" />{lang === 'ar' ? item.ar : item.en}</button>; })}
        </div>
        <div className="flex flex-wrap gap-2 px-4 py-3">
          {group.reports.map((key) => <button key={key} data-report-nav={key} onClick={() => selectReport(key)} className="rounded-ui-lg border border-ui-border bg-ui-page-alt px-3 py-2 text-xs font-medium text-ui-text transition hover:border-ui-primary hover:bg-ui-primary-soft hover:text-ui-primary">{lang === 'ar' ? REPORT_LABELS[key][0] : REPORT_LABELS[key][1]}</button>)}
        </div>
        {customizeOpen && (
          <div className="border-t border-ui-border bg-ui-page-alt px-4 py-3">
            <div className="mb-2 text-xs font-bold text-ui-text">{lang === 'ar' ? 'الأعمدة الظاهرة في التقرير الحالي' : 'Visible columns in the current report'}</div>
            <div className="flex flex-wrap gap-2">
              {columns.map((column, i) => <label key={`${column}-${i}`} className="flex items-center gap-2 rounded-ui-lg border border-ui-border bg-ui-surface px-3 py-2 text-xs text-ui-text"><input type="checkbox" checked={visible[i] !== false} onChange={() => setVisible((prev) => prev.map((v, index) => index === i ? !v : v))} />{column}</label>)}
            </div>
            {columns.length === 0 && <div className="text-xs text-ui-muted">{lang === 'ar' ? 'اختر تقريرًا لعرض أعمدته.' : 'Select a report to see its columns.'}</div>}
          </div>
        )}
      </div>
      <ReportsPage />
