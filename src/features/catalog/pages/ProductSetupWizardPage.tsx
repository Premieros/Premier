...
export function ProductSetupWizardPage() {
  ...
  return <DesignSurface testId="product-setup-wizard-page"><DesignPageHeader title={isAr ? 'إضافة منتج متكامل' : 'Complete Product Setup'} subtitle={isAr ? 'المنتج ← الوحدات ← طريقة الحصول عليها ← Recipe' : 'Product → Units → Sourcing → Recipe'} actions={<Button variant="outline" size="sm" onClick={() => navigate('/products')}><ChevronLeft className="w-4 h-4" />{isAr ? 'العودة للمنتجات' : 'Back to products'}</Button>} /><DesignPanel>
    <div className="flex items-center gap-2 mb-6 overflow-x-auto pb-1">{[['1', isAr ? 'المنتج' : 'Product'], ['2', isAr ? 'الوحدات' : 'Units'], ['3', isAr ? 'المكونات وRecipe' : 'Components & Recipe'], ['4', isAr ? 'مراجعة' : 'Review']].map(([n, label]) => { const active = Number(n) === step; const done = Number(n) < step; return <div key={n} className={`flex items-center gap-2 px-3 py-2 rounded-xl text-sm whitespace-nowrap ${active ? 'bg-brand-600 text-white' : done ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 dark:bg-slate-800 text-slate-500'}`}><span className="w-6 h-6 rounded-full flex items-center justify-center bg-white/20">{done ? <Check className="w-4 h-4" /> : n}</span>{label}</div>; })}</div>
