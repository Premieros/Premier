    ...
    <div className="space-y-5 pb-10">
      <div className="rounded-3xl bg-gradient-to-br from-slate-950 via-navy-900 to-slate-800 p-6 text-white shadow-xl">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <div className="mb-2 flex items-center gap-2 text-sm text-emerald-300"><ShieldCheck className="h-4 w-4" />System Health</div>
            <h1 className="text-3xl font-bold">{ar ? 'فحص سلامة النظام' : 'System Health Check'}</h1>
            <p className="mt-2 max-w-3xl text-sm text-slate-300">{ar ? 'فحص اتصال قاعدة البيانات والوصول إلى الجداول الأساسية بدون إنشاء أو تعديل أي بيانات.' : 'Checks database connectivity and access to core tables without creating or changing data.'}</p>
          </div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card className="p-5"><div className="text-sm text-slate-500">{ar ? 'يعمل' : 'Healthy'}</div><div className="mt-1 text-3xl font-bold text-emerald-600">{ok}</div></Card>
        <Card className="p-5"><div className="text-sm text-slate-500">{ar ? 'تحذيرات' : 'Warnings'}</div><div className="mt-1 text-3xl font-bold text-amber-600">{warnings}</div></Card>
        <Card className="p-5"><div className="text-sm text-slate-500">{ar ? 'أخطاء' : 'Errors'}</div><div className="mt-1 text-3xl font-bold text-red-600">{errors}</div></Card>
      </div>

      <Card className="overflow-hidden">
        <div className="flex items-center gap-3 border-b border-slate-100 p-5 dark:border-navy-800"><Database className="h-5 w-5 text-brand-600" /><div><h2 className="font-bold">{ar ? 'فحوص قاعدة البيانات' : 'Database checks'}</h2><p className="text-xs text-slate-400">{finishedAt ? `${ar ? 'آخر فحص' : 'Last check'}: ${finishedAt}` : ''}</p></div></div>
        <div className="divide-y divide-slate-100 dark:divide-navy-800">
          {checks.map(check => (
            <div key={check.key} className="flex items-center gap-4 p-4">
              {icon(check.status)}
              <div className="min-w-0 flex-1"><div className="font-semibold">{ar ? check.ar : check.en}</div><div className="truncate text-xs text-slate-400">{check.detail}</div></div>
              {check.status === 'ok' && typeof check.count === 'number' && <div className="rounded-lg bg-slate-100 px-3 py-1 text-sm font-semibold dark:bg-navy-800">{check.count.toLocaleString()}</div>}
            </div>
          ))}
        </div>
      </Card>
