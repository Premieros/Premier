      ...
      <div className="flex flex-wrap items-center gap-2">
        {(['monthly', 'yearly'] as Period[]).map((p) => (
          <button
            ...
            type="button"
            onClick={() => setPeriod(p)}
            className={`rounded-xl px-4 py-2 text-sm font-semibold transition ${period === p ? 'bg-brand-600 text-white shadow' : 'bg-slate-200 text-slate-600 hover:bg-slate-300 dark:bg-navy-800 dark:text-slate-300'}`}
          >
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {branches.map((b) => {
            ...
            const daysLeft = st?.status === 'trial' && st.trial_ends_at
              ? Math.max(0, Math.ceil((new Date(st.trial_ends_at).getTime() - Date.now()) / 86400000))
              : null;
              ...
              <Card key={b.id} className="p-5">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                  <div className="flex items-center gap-3">
                    <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-navy-900 text-white dark:bg-brand-600">
                      <Store className="h-5 w-5" />
                    </div>
                    <div>
                      <p className="font-bold text-slate-900 dark:text-white">{isAr ? b.name : (b.name_en || b.name)}</p>
                      <span className={`mt-1 inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-bold ${meta.cls}`}>
                        {st?.expired ? <AlertTriangle className="h-3 w-3" /> : <BadgeCheck className="h-3 w-3" />}
                        {meta.label}
                      </span>
                    </div>
                  </div>
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      <select
                        value={selPeriod}
                        onChange={(e) => setPeriodFor((prev) => ({ ...prev, [b.id]: e.target.value as Period }))}
                        className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm dark:border-navy-700 dark:bg-navy-800"
                      >
                      ...
                      <select
                        value=""
                        onChange={(e) => { if (e.target.value) void activate(b.id, e.target.value); e.target.value = ''; }}
                        className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm dark:border-navy-700 dark:bg-navy-800"
                      >
                        <option value="">{t('choosePlan')}…</option>
                        {plans.map((p) => (
                          <option key={p.id} value={p.id}>
                            {isAr ? p.name_ar : p.name_en} — {formatCurrency(selPeriod === 'monthly' ? p.monthly_price_egp : p.yearly_price_egp)}
                          </option>
                        ))}
                      </select>
