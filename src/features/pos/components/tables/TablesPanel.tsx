...
export function TablesPanel({ open, onClose, tables, ordersByTable, currency, onResume, onPay, onStart }: TablesPanelProps) {
  ...
  return (
    <>
      {open && <div className="fixed inset-0 top-16 z-40 bg-white dark:bg-navy-900 flex flex-col animate-fade-in">
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-navy-800 flex-shrink-0">
          <h2 className="text-sm font-bold text-slate-800 dark:text-white flex items-center gap-2">
            ...
            {t('tables')}
            <span className="px-2 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 text-[11px] font-bold">
          ...
          <div className="flex items-center gap-2">
            {canManage && (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-navy-800 text-slate-600 dark:text-slate-300 text-xs font-bold hover:bg-slate-200 dark:hover:bg-navy-700 transition-all active:scale-95"
              >
                ...
                areaTables.length === 0 ? null : (
                  <div key={area.id} className="space-y-2">
                    <h3 className="text-xs font-black text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
                      <MapPin className="w-3.5 h-3.5 text-brand-500 dark:text-gold-400" />
                      {area.name}
                      <span className="text-[10px] font-normal text-slate-400">({areaTables.length})</span>
                    </h3>
          ...
          return (
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-slate-400 flex items-center gap-1"><Users className="w-3.5 h-3.5" /> {selected.capacity}</span>
              </div>
                  ...
                  {tableOrders.map((order) => (
                    <div key={order.id} className="rounded-xl border border-slate-100 dark:border-navy-800 bg-slate-50 dark:bg-navy-800/50 p-3 space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-sm font-bold text-brand-600 dark:text-gold-400">{formatCurrency(order.total, currency, lang)}</span>
                      </div>
