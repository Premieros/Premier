            ...
            return (
              <div key={send.id} className="flex items-center justify-between text-sm">
                <span className="flex items-center gap-2 min-w-0">
                  ...
                  <span className="shrink-0 text-[11px] font-bold text-orange-600 dark:text-orange-300">× {Number(item?.quantity ?? 0)}</span>
                </span>
                <span className="shrink-0 text-[11px] text-slate-400 tabular-nums">{formatClockTime(send.sent_at, lang)}</span>
              </div>
            );
          })}
        </div>
      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-navy-800 flex-shrink-0">
          <h2 className="text-sm font-bold text-slate-800 dark:text-white flex items-center gap-2">
            ...
            {t('kitchen')}
            <span className="px-2 py-0.5 rounded-full bg-orange-50 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300 text-[11px] font-bold">{withSends.length}</span>
          </h2>
