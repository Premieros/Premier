...
export function TableActionModal({ table, onClose, orders, itemsByOrder, kitchenSendsByOrder, currency, onStart, onResume, onPay }: TableActionModalProps) {
  ...
  return (<Modal open={!!table} onClose={onClose} title={table.name} size="md"><div className="space-y-4">
    <div className="flex items-center gap-2"><span className={`w-2.5 h-2.5 rounded-full ${st.dot}`} /><span className={`px-2.5 py-1 rounded-full text-xs font-bold ${st.badge}`}>{t(st.label)}</span><span className="text-xs text-slate-400 flex items-center gap-1"><Users className="w-3.5 h-3.5" /> {table.capacity}</span></div>
    {tableOrders.length > 0 ? <div className="space-y-2"><p className="text-xs font-bold text-slate-400">{isAr ? 'طلب موجود على الطاولة' : 'Existing order on this table'}</p>{tableOrders.map((order) => { const stage = stageOfOrder(order, itemsByOrder, kitchenSendsByOrder); const itemCount = (itemsByOrder[order.id] || []).reduce((s, i) => s + Number(i.quantity), 0); return <div key={order.id} className="rounded-xl border border-slate-100 dark:border-navy-800 bg-slate-50 dark:bg-navy-800/50 p-3 space-y-2"><div className="flex items-center justify-between gap-2"><span className="flex items-center gap-2 min-w-0"><span className="text-sm font-bold text-slate-800 dark:text-white">{order.order_number}</span><OrderTypePill type={order.order_type as Order['order_type']} /><OrderStageBadge stage={stage} /></span><span className="text-sm font-bold text-brand-600 dark:text-gold-400 shrink-0">{formatCurrency(order.total, currency, lang)}</span></div><div className="text-[11px] text-slate-400">{isAr ? `${itemCount} صنف` : `${itemCount} items`}</div><div className="flex flex-wrap gap-2 pt-1"><Button size="sm" variant="outline" data-testid={`pos-table-${table.id}-resume-${order.id}`} onClick={() => { onClose(); onResume(order); }}><Play className="w-3.5 h-3.5" /> {t('resumeOrder')}</Button><Button size="sm" variant={stage === 'ready' ? 'success' : 'outline'} data-testid={`pos-table-${table.id}-pay-${order.id}`} onClick={() => { onClose(); onPay(order); }}><Banknote className="w-3.5 h-3.5" /> {t('payOrder')}</Button></div></div>; })}</div>
    : canStart ? <div className="space-y-3"><div className="flex items-center gap-3"><label className="text-sm font-medium text-slate-700 dark:text-slate-300">{t('guestCount')}:</label><input data-testid={`pos-table-${table.id}-guest-count`} type="number" min={1} value={guests || ''} onChange={(e) => setGuests(parseInt(e.target.value) || 1)} className="w-24 px-3 py-2 rounded-xl border border-slate-200 dark:border-navy-700 bg-white dark:bg-navy-800 text-sm font-bold text-slate-800 dark:text-white focus:ring-2 focus:ring-gold-500/50" /></div><Button size="lg" className="w-full" data-testid={`pos-table-${table.id}-start`} onClick={() => { onClose(); onStart(Math.max(1, guests)); }}><UtensilsCrossed className="w-5 h-5" /> {t('startOrder')}</Button></div>
    : <p className="text-sm text-slate-500 dark:text-slate-400">{isAr ? 'هذه الطاولة غير متاحة حالياً.' : 'This table is not available right now.'}</p>}
  </div></Modal>);
}
