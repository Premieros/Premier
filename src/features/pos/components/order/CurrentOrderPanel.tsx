  ...
  return <div className="flex flex-col h-full min-h-0 bg-white dark:bg-navy-900">
    <div className="px-3 py-2.5 border-b border-slate-100 dark:border-navy-800 flex-shrink-0 space-y-2">
      <div className="flex items-center gap-2">
        ...
        {activeOrderId && !empty && <button onClick={onClear} aria-label={isAr ? 'مسح الطلب' : 'Clear order'} className="text-[11px] text-red-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 px-2 py-1 rounded-lg transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>}
      </div>
    ...
    <div className="flex-1 overflow-y-auto px-2.5 py-2">
      {empty ? <div className="flex flex-col items-center justify-center h-full text-slate-400"><div className="w-20 h-20 rounded-full bg-slate-100 dark:bg-navy-800 flex items-center justify-center mb-3"><ShoppingCart className="w-10 h-10 text-slate-300 dark:text-navy-700" /></div><p className="text-sm font-medium">{t('emptyCart')}</p><p className="text-xs text-slate-400 mt-1">{isAr ? 'اضغط على المنتج لإضافته' : 'Tap a product to add it'}</p></div> : <div className="space-y-1.5">{cart.map((item) => {
        ...
        return <div key={item.product.id} className="flex items-center gap-2 p-2 rounded-xl bg-slate-50 dark:bg-navy-800/50 hover:bg-slate-100 dark:hover:bg-navy-800 transition-colors group">
          ...
          <div className="flex items-center gap-1"><button data-testid={`pos-cart-qty-decrease-${item.product.id}`} aria-label={isAr ? `تقليل كمية ${item.product.name}` : `Decrease quantity ${item.product.name}`} onClick={() => onUpdateQty(item.product.id, -1)} className="h-7 w-7 rounded-lg border border-slate-200 dark:border-navy-700 flex items-center justify-center"><Minus className="w-3.5 h-3.5" /></button><span data-testid={`pos-cart-qty-${item.product.id}`} className="w-6 text-center text-xs font-black">{item.quantity}</span><button data-testid={`pos-cart-qty-increase-${item.product.id}`} aria-label={isAr ? `زيادة كمية ${item.product.name}` : `Increase quantity ${item.product.name}`} onClick={() => onUpdateQty(item.product.id, 1)} className="h-7 w-7 rounded-lg bg-gold-500 text-navy-950 flex items-center justify-center"><Plus className="w-3.5 h-3.5" /></button></div>
          <span className="w-20 text-end text-xs font-black">{formatCurrency(item.quantity * item.unit_price - (item.discount_amount || 0), currency, lang)}</span><button onClick={() => onRemove(item.product.id)} aria-label={isAr ? `حذف ${item.product.name}` : `Remove ${item.product.name}`} className="p-1 text-slate-300 hover:text-red-500"><X className="w-4 h-4" /></button>
        </div>;
      })}</div>}
    </div>

    <div className="border-t border-slate-100 dark:border-navy-800 p-3 flex-shrink-0 space-y-2">
      <div className="grid grid-cols-3 gap-2"><div className="rounded-xl bg-slate-50 p-2 dark:bg-navy-800"><p className="text-[10px] text-slate-400">{t('subtotal')}</p><p className="text-xs font-black">{formatCurrency(subtotal, currency, lang)}</p></div><div className="rounded-xl bg-slate-50 p-2 dark:bg-navy-800"><p className="text-[10px] text-slate-400">{t('discount')}</p><p data-testid="pos-discount-value" className="text-xs font-black">{formatCurrency(discountValue, currency, lang)}</p></div><div className="rounded-xl bg-gold-50 p-2 dark:bg-gold-900/20"><p className="text-[10px] text-slate-400">{t('total')}</p><p data-testid="pos-total-value" className="text-sm font-black text-gold-600">{formatCurrency(total, currency, lang)}</p></div></div>
      <div className="flex gap-2"><button data-testid="pos-action-discount" aria-label={isAr ? 'الخصم' : 'Discount'} onClick={() => setShowDiscount(!showDiscount)} className="flex-1 rounded-xl bg-slate-100 py-2 text-xs font-black dark:bg-navy-800"><Percent className="mx-auto h-4 w-4" /></button><button data-testid="pos-action-hold" aria-label={isAr ? 'تعليق الطلب' : 'Hold order'} onClick={onHold} disabled={empty || orderLoading} className="flex-1 rounded-xl bg-slate-100 py-2 text-xs font-black disabled:opacity-40 dark:bg-navy-800"><Pause className="mx-auto h-4 w-4" /></button><button data-testid="pos-action-send-kitchen" aria-label={isAr ? 'إرسال للمطبخ' : 'Send to kitchen'} onClick={onSendKitchen} disabled={empty || kitchenSending || allSent} className="flex-1 rounded-xl bg-slate-100 py-2 text-xs font-black disabled:opacity-40 dark:bg-navy-800"><ChefHat className="mx-auto h-4 w-4" /></button><button data-testid="pos-action-print" aria-label={isAr ? 'طباعة' : 'Print'} onClick={onPrint} disabled={empty} className="flex-1 rounded-xl bg-slate-100 py-2 text-xs font-black disabled:opacity-40 dark:bg-navy-800"><Printer className="mx-auto h-4 w-4" /></button><button data-testid="pos-action-pay" aria-label={isAr ? 'الدفع' : 'Pay'} onClick={onPay} disabled={empty || completing} className="flex-[2] rounded-xl bg-emerald-600 py-2 text-xs font-black text-white disabled:opacity-40"><Banknote className="mx-auto h-4 w-4" />{isAr ? 'الدفع' : 'Pay'}</button></div>
      {showDiscount && <div data-testid="pos-discount-editor" className="rounded-xl border border-slate-200 p-2 dark:border-navy-700"><div className="flex gap-2"><button data-testid="pos-discount-percent" onClick={() => onDiscountTypeChange('percent')} className="flex-1 rounded-lg bg-slate-100 p-2 text-xs font-black dark:bg-navy-800">%</button><button data-testid="pos-discount-amount" onClick={() => onDiscountTypeChange('amount')} className="flex-1 rounded-lg bg-slate-100 p-2 text-xs font-black dark:bg-navy-800">{currency}</button><input data-testid="pos-discount-input" aria-label={isAr ? 'قيمة الخصم' : 'Discount value'} type="number" value={discountAmount || ''} onChange={(e) => onDiscountAmountChange(parseFloat(e.target.value) || 0)} className="w-24 rounded-lg border p-2 text-center text-xs dark:border-navy-700 dark:bg-navy-800" /></div></div>}
    </div>
  </div>;
}