...
export function SubscriptionPage() {
  ...
  const submitPayment = async () => {
    ...
    if (error || !(data as { success?: boolean })?.success) {
      show((data as { error?: string })?.error || error?.message || (isAr ? 'تعذر إرسال الطلب' : 'Payment submission failed'), 'error');
      return;
    }
  ...
  return <div className="space-y-6 pb-10">
    <div className="rounded-3xl bg-gradient-to-br from-slate-950 via-navy-900 to-slate-800 p-6 text-white shadow-xl">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div><div className="mb-2 flex items-center gap-2 text-sm text-gold-300"><CreditCard className="h-4 w-4" />{isAr ? 'الاشتراك والدفع' : 'Subscription & Billing'}</div><h1 className="text-3xl font-bold">{isAr ? 'اشتراك Premier' : 'Premier Subscription'}</h1><p className="mt-2 text-sm text-slate-300">{isAr ? 'اختر خطتك وحوّل قيمة الاشتراك عبر InstaPay.' : 'Choose your plan and pay by InstaPay transfer.'}</p></div>
        <div className="inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-2 text-sm font-bold"><BadgeCheck className="h-4 w-4" />{statusLabel}</div>
      </div>
    </div>
