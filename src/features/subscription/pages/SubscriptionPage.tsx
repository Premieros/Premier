...
export function SubscriptionPage() {
  ...
  return <div className="space-y-6 pb-10">
    ...
    <Card className="p-5"><div className="mb-5 flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-bold">{isAr ? 'اختر الخطة' : 'Choose a plan'}</h2><p className="text-sm text-slate-400">{isAr ? 'الدفع يتم يدويًا عبر InstaPay ويعتمد بعد المراجعة.' : 'Payment is manual via InstaPay and activated after review.'}</p></div><div className="flex rounded-xl bg-slate-100 p-1 dark:bg-navy-800">{(['monthly','yearly'] as Period[]).map(p => <button key={p} onClick={() => setPeriod(p)} disabled={p === 'monthly' ? !settings?.allow_monthly : !settings?.allow_yearly} className={`rounded-lg px-4 py-2 text-sm font-semibold ${period === p ? 'bg-white shadow dark:bg-navy-700' : 'text-slate-500'} disabled:cursor-not-allowed disabled:opacity-40`}>{p === 'monthly' ? (isAr ? 'شهري' : 'Monthly') : (isAr ? 'سنوي' : 'Yearly')}</button>)}</div></div><div className="grid gap-4 md:grid-cols-3">{plans.map(plan => <div key={plan.id} className="flex flex-col rounded-2xl border border-slate-200 p-5 dark:border-navy-700"><h3 className="text-lg font-bold">{isAr ? plan.name_ar : plan.name_en}</h3><p className="mt-2 text-3xl font-black">{period === 'monthly' ? plan.monthly_price_egp : plan.yearly_price_egp}<span className="text-sm font-medium text-slate-400"> EGP</span></p><ul className="mt-4 flex-1 space-y-2">{Array.isArray(plan.features) && plan.features.map((f, i) => <li key={i} className="flex gap-2 text-sm text-slate-600 dark:text-slate-300"><Check className="h-4 w-4 text-emerald-500" />{f}</li>)}</ul><Button className="mt-5 w-full" disabled={!instapay} onClick={() => setSelected(plan)}><WalletCards className="h-4 w-4" />{isAr ? 'الدفع عبر InstaPay' : 'Pay with InstaPay'}</Button></div>)}</div></Card>
    {selected && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4"><div className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl dark:bg-navy-900"><div className="flex items-start justify-between"><div><h2 className="text-xl font-bold">{isAr ? 'تحويل InstaPay' : 'InstaPay transfer'}</h2><p className="mt-1 text-sm text-slate-500">{isAr ? 'الخطة' : 'Plan'}: {isAr ? selected.name_ar : selected.name_en}</p></div><button onClick={() => setSelected(null)} className="text-slate-400">×</button></div><div className="mt-5 rounded-2xl bg-slate-50 p-4 dark:bg-navy-800"><p className="text-xs text-slate-500">{isAr ? 'المبلغ المطلوب' : 'Amount due'}</p><p className="text-3xl font-black">{period === 'monthly' ? selected.monthly_price_egp : selected.yearly_price_egp} EGP</p><div className="mt-3 flex items-center justify-between rounded-xl bg-white p-3 dark:bg-navy-700"><div className="min-w-0"><p className="text-xs text-slate-400">InstaPay</p><span className="font-bold break-all">{instapay || (isAr ? 'لم يتم ضبط الحساب' : 'Account not configured')}</span>{settings?.beneficiary_name && <p className="text-xs text-slate-500">{settings.beneficiary_name}</p>}</div><button onClick={copyInstaPay} disabled={!instapay}><Copy className="h-4 w-4" /></button></div>{settings?.instructions_ar || settings?.instructions_en ? <p className="mt-3 text-sm text-slate-600 dark:text-slate-300">{isAr ? settings.instructions_ar : settings.instructions_en}</p> : null}{settings?.qr_code_url ? <img src={settings.qr_code_url} alt="InstaPay QR" className="mx-auto mt-4 h-40 w-40 rounded-xl border object-contain" /> : null}</div><div className="mt-4 space-y-3"><input value={reference} onChange={e => setReference(e.target.value)} placeholder={isAr ? 'مرجع التحويل' : 'Transfer reference'} className="w-full rounded-xl border p-3 dark:border-navy-700 dark:bg-navy-800"/><input value={receiptUrl} onChange={e => setReceiptUrl(e.target.value)} placeholder={isAr ? 'رابط صورة الإيصال (اختياري)' : 'Receipt image URL (optional)'} className="w-full rounded-xl border p-3 dark:border-navy-700 dark:bg-navy-800"/><div className="flex gap-2"><Button variant="outline" onClick={() => setSelected(null)} className="flex-1">{isAr ? 'إلغاء' : 'Cancel'}</Button><Button onClick={submitPayment} disabled={submitting || !instapay} className="flex-1">{submitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Upload className="h-4 w-4" />}{isAr ? 'إرسال للمراجعة' : 'Submit for review'}</Button></div></div></div></div>}
  </div>;
}
