...
const variants: Record<Variant, string> = {
  primary: 'bg-teal-600 hover:bg-teal-700 text-white shadow-sm active:bg-teal-800',
...
export function Button({ variant = 'primary', size = 'md', children, className = '', ripple = true, onClick, ...props }: ButtonProps) {
  ...
  return (
    <button
      ref={btnRef}
      className={`inline-flex items-center justify-center gap-2 font-medium transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-1 dark:focus:ring-offset-slate-900 disabled:opacity-50 disabled:cursor-not-allowed ${variants[variant]} ${sizes[size]} ${className}`}
      onClick={handleClick}
      {...props}
    >
      {children}
