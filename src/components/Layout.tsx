...
export function Layout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
      {/* Sidebar */}
      <aside
        className={`fixed top-0 ${lang === 'ar' ? 'right-0' : 'left-0'} z-40 h-full w-64 bg-white dark:bg-slate-900 ${lang === 'ar' ? 'border-l' : 'border-r'} border-slate-200 dark:border-slate-800 transform transition-transform duration-300 ${
          sidebarOpen
            ? 'translate-x-0'
            : lang === 'ar'
              ? 'translate-x-full'
          ...
          {Object.entries(grouped).map(([group, items]) => (
            <div key={group} className="mb-1">
              <button
                ...
                className="flex items-center justify-between w-full px-3 py-1.5 text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider hover:text-slate-600 dark:hover:text-slate-300 transition-colors"
              >
      ...
      <div className={`${lang === 'ar' ? 'lg:mr-64' : 'lg:ml-64'}`}>
        {/* Header */}
        <header className="sticky top-0 z-20 h-16 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-2 rounded-xl text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-700 dark:hover:text-slate-300 transition-colors"
