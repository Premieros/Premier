-- Kitchen security hardening
-- SECURITY DEFINER RPCs must never trust a caller-supplied branch_id/order_id.
-- Keep Super Admin/enterprise admins able to inspect explicitly selected branches,
-- while branch-scoped users are locked to their own branch.
...
BEGIN
  IF NOT public.is_pos_admin() AND p_branch_id IS DISTINCT FROM v_user_branch THEN
...
BEGIN
  ...
  IF NOT public.is_pos_admin() AND v_order_branch IS DISTINCT FROM public.get_branch_id() THEN
    RAISE EXCEPTION 'BRANCH_ACCESS_DENIED';
  END IF;
