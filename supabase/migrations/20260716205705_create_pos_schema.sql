/*
# POS System - Complete Schema

## Overview
Creates a full Restaurant & Retail Point-of-Sale system with multi-branch,
multi-warehouse support, products with multiple units, sales, purchases,
customers, suppliers, expenses, users, audit log, and settings.

## New Tables
1. branches - Store branches/locations
2. warehouses - Storage locations linked to branches
3. categories - Product categories
4. products - Product catalog with barcode, cost, price
5. product_units - Multiple units per product (carton, box, pack, piece, kg, liter) with conversion factors
6. inventory - Stock quantities per product per warehouse
7. customers - Customer records
8. suppliers - Supplier records
9. sales - Sales transaction headers (POS invoices)
10. sale_items - Line items for each sale
11. purchases - Purchase order headers
12. purchase_items - Line items for each purchase
13. expenses - Expense records
14. users - App user profiles linked to auth.users, with role + branch assignment
15. audit_log - Action audit trail
16. settings - System-wide settings (single row)

## Security
- RLS enabled on ALL tables.
- All authenticated POS staff share organization data (not per-user isolated).
- Policies use TO authenticated with USING(true) / WITH CHECK(true) because
  the data is intentionally shared among all signed-in staff.
- audit_log INSERT allows authenticated; SELECT/DELETE restricted.
*/
