...
ALTER TABLE public.raw_materials ADD CONSTRAINT raw_materials_branch_id_fkey FOREIGN KEY (branch_id) REFERENCES public.branches(id) ON DELETE RESTRICT;
...
CREATE POLICY raw_materials_select_branch_isolated ON public.raw_materials FOR SELECT USING (public.is_super_admin() OR branch_id = public.user_branch_id());
CREATE POLICY raw_materials_insert_branch_isolated ON public.raw_materials FOR INSERT WITH CHECK (public.is_super_admin() OR branch_id = public.user_branch_id());
CREATE POLICY raw_materials_update_branch_isolated ON public.raw_materials FOR UPDATE USING (public.is_super_admin() OR branch_id = public.user_branch_id()) WITH CHECK (public.is_super_admin() OR branch_id = public.user_branch_id());
CREATE POLICY raw_materials_delete_branch_isolated ON public.raw_materials FOR DELETE USING (public.is_super_admin() OR branch_id = public.user_branch_id());

CREATE OR REPLACE FUNCTION public.validate_recipe_item_branch_match()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE recipe_branch uuid; material_branch uuid;
BEGIN
 ...
 IF recipe_branch IS NULL OR material_branch IS NULL OR recipe_branch <> material_branch THEN RAISE EXCEPTION 'RAW_MATERIAL_BRANCH_MISMATCH'; END IF;
 RETURN NEW;
END;
$$;
DROP TRIGGER IF EXISTS trg_validate_recipe_item_branch ON public.recipe_items;
CREATE TRIGGER trg_validate_recipe_item_branch BEFORE INSERT OR UPDATE ON public.recipe_items FOR EACH ROW EXECUTE FUNCTION public.validate_recipe_item_branch_match();
