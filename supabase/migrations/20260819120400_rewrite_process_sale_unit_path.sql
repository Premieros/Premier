-- Finalize the POS sale path: product inventory_batches are never deducted by sales.
-- Unit batches are authoritative; raw materials remain manufacturing-only.
...
BEGIN
  ...
  IF v_src IS NULL THEN RAISE EXCEPTION 'target process_sale not found'; END IF;
...
$old$;
  v_new := $new$
      ...
      v_cogs_total := v_cogs_total + COALESCE((v_res->>'total_cost')::numeric, 0);
$new$;
  IF position(v_old in v_src) = 0 THEN RAISE EXCEPTION 'product deduction write block not found'; END IF;
  ...
  v_old := $old$
      ...
      END IF;
$old$;
  IF position(v_old in v_src) = 0 THEN RAISE EXCEPTION 'product stock validation block not found'; END IF;
