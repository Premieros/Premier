...
describe('computeSentState', () => {
  ...
  it('shows partial state when one order line for a product is sent and another is new', () => {
    const burger = product('burger');
    const cart = [cartItem(burger, 2)];
    const items = [orderItem('sent-line', 'burger'), orderItem('new-line', 'burger')];

    const state = computeSentState(
      cart,
      items,
      new Set(['sent-line']),
      [kitchenSend('sent-line', 'burger')],
    );

    expect(state.burger.sentQty).toBe(1);
    expect(state.burger.newQty).toBe(1);
    expect(state.burger.sent).toBe(false);
    expect(state.burger.partial).toBe(true);
  });
});
