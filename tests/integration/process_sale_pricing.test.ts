...
// Functional test for the D13 authoritative-pricing hardening of process_sale.
//
...
describe.skipIf(skip)('process_sale authoritative pricing (D13)', () => {
  ...
  beforeAll(async () => {
    ...
    // The default seed enables VAT (15%): disable it so the totals asserted
    // below are exactly the authoritative catalog prices, not price + tax.
  ...
  it('books catalog price, not the client-supplied price', async () => {
    // unit_price = 1 and total = 2 are forged: the real price is 100.
    const res = await client.query(
      ...
      [
        ...
        warehouseId,
        JSON.stringify([
          { product_id: productId, unit_name: 'piece', quantity: 2, unit_price: 1, discount_amount: 0, bonus_quantity: 0, total: 2 },
        ]),
      ],
    );
    ...
    const items = await client.query(
      `SELECT unit_price, quantity, discount_amount, total FROM public.sale_items WHERE sale_id = $1`,
      [r.sale_id],
    );
    ...
    const sale = await client.query(
      `SELECT subtotal, discount_amount, tax_amount, total, paid_amount FROM public.sales WHERE id = $1`,
      [r.sale_id],
    );
    ...
    const journal = await client.query(
      `SELECT
         COALESCE(SUM(l.debit), 0)::numeric(14,2)  AS dr,
         COALESCE(SUM(l.credit), 0)::numeric(14,2) AS cr
  ...
  it('rejects a discount that exceeds the line total (clamped server-side)', async () => {
    ...
    const res = await client.query(
      `SELECT public.process_sale($1, $2, $3, NULL, NULL, 0, 0, 'amount', 0, 0, 0, 0, 'cash', 'completed',
         $4::jsonb) AS r`,
      [
        inv2,
        branchId,
        warehouseId,
        JSON.stringify([
          { product_id: productId, unit_name: 'piece', quantity: 1, unit_price: 100, discount_amount: 500, bonus_quantity: 0, total: -400 },
        ]),
      ],
    );
    const r = res.rows[0].r;
    ...
    const items = await client.query(
      `SELECT unit_price, discount_amount, total FROM public.sale_items WHERE sale_id = $1`,
      [r.sale_id],
    );
    expect(Number(items.rows[0].unit_price)).toBe(100);
    expect(Number(items.rows[0].discount_amount)).toBe(100); // clamped to qty*price
