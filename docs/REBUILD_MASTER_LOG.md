...
- **ERP-01 PHASE F verification (in progress — re-verified 2026-08-14):** fresh `npm ci` ✅ (480 packages) + all local gates green (lint 0 errors / 16 pre-existing warnings, typecheck, typecheck:all, test:unit 236/236 across 19 files, build). **Fix recorded:** `@playwright/test@^1.62.1` declared as devDependency (commit `32d2faa`) — was used by `tests/e2e` but never declared; `npm ci`+`typecheck:all` now deterministic. **Environment-blocked, NOT executed (no `.env`, no Postgres/Docker, no values invented):** `npm run test:integration` self-skips without `SUPABASE_DB_URL`/`DATABASE_URL` → recorded as **NOT EXECUTED — ENVIRONMENT BLOCKED** (10 files / 154 tests skipped). Browser E2E (`pos-actions` 9, `public-smoke` 38 — 2 + 36 protected routes, `dashboard-navigation` 3 = 50 tests) needs `VITE_SUPABASE_URL` (exactly the specs' mock origin `https://lwnsdsncmlsroiswgoga.supabase.co`) + non-empty `VITE_SUPABASE_ANON_KEY` at build time (`src/lib/supabase.ts` throws synchronously otherwise) — Playwright 1.62.1 + chromium-1234 already installed. Static review of migration `069` passed (function-only replacement; no RLS/grants/schema change; branch guard identical to 046; send boundary untouched) — live confirmation pending the DB suite. Then close PHASE G docs, final review and PR. Do NOT start ERP-02 or open a PR until PHASE F and G are fully closed.
