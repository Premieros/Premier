...
- **Multi-Tenant Phase 3 (Full Tenant Data Isolation) — IN PROGRESS.** PR #10 extended with `bbf4598`. Migration `20260821010000_tenant_data_isolation.sql`: `user_may_access_branch(uuid)` SECURITY DEFINER helper (org-membership check + NULL branch fallback for platform admin). Updated RLS policies on 40+ branch-scoped tables: SELECT uses `user_may_access_branch(branch_id)`; WRITE uses `is_platform_admin() OR (can_permission(...) AND user_may_access_branch(branch_id))`. Child tables (order_items, journal_entry_lines, bank_statement_lines, shift_operations, warehouse_transfer_items, recipe_items, product_components, product_units, order_kitchen_sends) use parent JOIN to branch_id. New integration test `tenant_data_isolation.test.ts` (30+ cases: cross-tenant SELECT isolation, write denial, branch_manager restrictions, super_admin full access, NULL branch fallback). `seedRlsFixture` updated: adds organizations + org_memberships for proper tenant isolation. CI pending.
...
- **Multi-Tenant Phase 3 CI pending** — PR #10 (agent/multitenant-phase2) now has 13 commits. Awaiting CI green (verify, db, browser-smoke) for Phase 3 tenant data isolation. After green: report final status, then merge.
